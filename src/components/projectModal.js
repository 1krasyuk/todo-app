...
class ProjectModal {
  ...
  initEventListeners() {
    document.getElementById('btn-add-project').addEventListener('click', () => this.open());

    document.getElementById('close-modal').addEventListener('click', () => this.close());
    document.getElementById('cancel-project').addEventListener('click', () => this.close());
  ...
  handleSubmit() {
    ...
    if (this.currentProject) {
      ...
      const updatedProject = projectManager.updateProject(this.currentProject.id, name);
      if (updatedProject) {
        renderAllProjects(projectManager.getAllProjects());
        selectProject(updatedProject, document.querySelector(`.project-item[data-id="${updatedProject.id}"]`));
      }
    } else {
      // Add new project
      const newProject = projectManager.addProject(name);
      renderProject(newProject);
      selectProject(newProject, document.querySelector(`.project-item[data-id="${newProject.id}"]`));
    }
